...
start = time.time_ns()
corr = wifi6_detect.wifi6_detect(xs_norm)
end = time.time_ns()
print(corr)
...
def wifi6_func():
    global wifi6_buffer, rx_cnt, dvbEvent,wifiCount
    flag = False
    totalTime = 0
    while rx_cnt< 200:
        wifiCount += 1
...
while rx_cnt<200:
    ...
    wifi6_buffer = np.hstack((wifi6_buffer[Nrx_buffer:], recv_buffer[0].copy()))
