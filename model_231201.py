...
class GenTx(MyWindow):
    ...
    def DVBT2_func(self):
        ...
        while rx_count < rx_count_End:
            ...
            # sg.resample(test[:Ndvbt2], int(len(Ndvbt2)*16/35))
            dvbt2_result[-1, 0] = dvbt2_detect.dvbt2_detect(sg.resample(test[:Ndvbt2], int(Ndvbt2*16/35)))
            # (test[Ndvbt2//2:15000])
            start = time.time_ns()
            dvbt2_result[-1, 1] = dvbt2_detect.dvbt2_detect(sg.resample(test[Ndvbt2//2:15000], int(Ndvbt2*16/35)))
