...
class GenTx(MyWindow):
    ...
    def usrpConfig(self):
        ...
        rf_freq = self.model_value['centerFreq']  # 2.62GHz carrier frequency
        tx_channel = 0
        txGain = self.model_value['txGain'] # dB
        rx_channel = 0
        rxGain = self.model_value['txGain']  # dB
        ...
        self.myusrp.set_tx_antenna("TX/RX0")
        self.myusrp.set_tx_rate(Fs)
        # self.myusrp.set_tx_bandwidth(Fs/2)
        self.myusrp.set_tx_freq(uhd.libpyuhd.types.tune_request(rf_freq), 0)
        self.myusrp.set_tx_gain(txGain)
        
        self.myusrp.set_rx_antenna("RX1")
        self.myusrp.set_rx_rate(Fs)
        # self.myusrp.set_rx_bandwidth(Fs/2)
        self.myusrp.set_rx_freq(uhd.libpyuhd.types.tune_request(rf_freq), 0)
    ...
    def Generation(self):
        ...
        if self.finalIndexWifi == 0 or self.finalIndexDvbt2 == 0:
            ...
            self.temp1 = self.wifiRectangles
            self.temp2 = self.dvbt2Rectangles
            wifi6_seq1 = self.wifiRectangles
            dvbt2_seq1 = self.dvbt2Rectangles
            self.drawRecursiveRectangles()
            print('complete.')
            tx_wifi6 = np.zeros(2000*Nseq_wifi6, dtype=np.complex64)
            tx_dvbt2 = np.zeros(10000*Nseq_dvbt2, dtype=np.complex64)
    ...
    def DVBT2_func(self):
        ...
        while rx_count < rx_count_End:
            ...
            test= (dvbt2_buffer-self.wifi6DC)/self.wifi6Std
    ...
    def calculation(self):
        ...
        wifi_start_idx = np.where(wifi6_seq ==1)[0][-20]
        dvbt2_start_idx = np.where(dvbt2_seq ==1)[0][-2]
        ...
        print(f"wifi6_offset: {wifi6_offset}, {wifi6_offset*2000}")
        print(f"dvbt2_offset: {dvbt2_offset}, {dvbt2_offset*5*2000}")
        #####
        print("################################")
        print(f'wifiDC : {tmp[wifi6_offset+wifi_start_idx:wifi6_offset+wifi_start_idx+1].mean()}')
        print(f'wifiStd : {tmp[wifi6_offset+wifi_start_idx:wifi6_offset+wifi_start_idx+1].std()}')
        print("################################")

        plt.figure()
        plt.subplot(211)
        plt.plot(tmp[wifi6_offset+wifi_start_idx:wifi6_offset+wifi_start_idx+1].flatten().real)
        plt.subplot(212)
        plt.plot(tmp[(dvbt2_offset+dvbt2_start_idx)*5:(dvbt2_offset+dvbt2_start_idx)*5+5].flatten().real)
        plt.show()
